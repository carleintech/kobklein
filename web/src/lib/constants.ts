...
export const CURRENCIES = {
  HTG: {
    ...
    name: 'Haitian Gourde',
    decimals: 2,
    exchangeRate: 132.50, // 1 USD = 132.50 HTG (example rate)
  },
  USD: {
    code: 'USD',
    symbol: '$',
...
// ===== USER ROLE CONSTANTS =====
export const USER_ROLES: Record<UserRole, { name: string; color: string; permissions: string[] }> = {
  client: {
    name: 'Client',
    color: 'blue',
    permissions: ['wallet:read', 'wallet:send', 'wallet:receive', 'transactions:read'],
  },
  merchant: {
    name: 'Merchant',
    color: 'green',
    permissions: ['wallet:read', 'wallet:receive', 'transactions:read', 'pos:use', 'payout:request'],
  },
  distributor: {
    name: 'Distributor',
    color: 'purple',
    permissions: ['cards:issue', 'refill:process', 'users:onboard', 'commission:view'],
  },
  diaspora: {
    name: 'Diaspora',
    color: 'orange',
    permissions: ['refill:send', 'beneficiaries:manage', 'auto-refill:configure'],
  },
  admin: {
    name: 'Admin',
    color: 'red',
    permissions: ['users:manage', 'transactions:view', 'reports:generate', 'system:configure'],
  },
  super_admin: {
    name: 'Super Admin',
    color: 'gray',
