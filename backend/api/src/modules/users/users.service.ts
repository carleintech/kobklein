...
export class UsersService {
  ...
  async getUserStats(userId: string) {
    ...
    try {
      ...
      // Calculate wallet balances from ledger (bank-grade)
      const totalBalance = await this.calculateUserTotalBalanceFromLedger(user.id);
  ...
  /**
   * Calculate total user balance from ledger (bank-grade)
   * Replaces old wallet.balance summation
   */
  private async calculateUserTotalBalanceFromLedger(userId: string): Promise<number> {
    try {
      ...
      if (error) {
        this.logger.error('Failed to fetch user accounts', error);
        return 0;
      }
      ...
      // Get balance for each account from ledger entries
      ...
      this.logger.error('Failed to calculate user total balance', error);
      return 0;
    }
  }
}
