...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    try {
      ...
      // ✅ AUTO-CREATE WALLETS for new user
      try {
        // Create HTG wallet (primary)
        await this.walletsService.createWallet({
          userId: user.id,
          currency: 'HTG',
        });
        this.logger.log(`HTG wallet created for user: ${user.id}`);
        ...
        this.logger.log(`USD wallet created for user: ${user.id}`);
      } catch (walletError) {
        this.logger.error(`Failed to create wallets for ${user.id}:`, walletError);
        // Don't block registration if wallet creation fails
        // User can contact support to create wallet manually
      }
  ...
  async getUserFromToken(token: string) {
    try {
      ...
      if (error || !data?.user) {
        throw new BadRequestException('Unable to retrieve Supabase user from token');
      }

      return data.user;
    } catch (error) {
      const err = extractError(error);
      this.logger.error(`Failed to get user from token:`, err.message);
      throw error;
    }
  }

  async syncUserFromSupabase(userData: { id: string; email: string; firstName?: string; lastName?: string; phoneNumber?: string; role?: UserRole }) {
    try {
      const { id, email, firstName, lastName, phoneNumber, role = UserRole.INDIVIDUAL } = userData;

      this.logger.log(`Syncing user from Supabase: ${email} (${id})`);
      ...
      if (existingUser) {
        ...
        return {
          user: this.mapDatabaseUserToProfile(updatedUser),
        ...
        const { data: newUser, error } = await this.supabase
          ...
          .insert({
            ...
            preferred_currency: 'HTG' as CurrencyCode,
            country: 'HT',
        ...
        return {
          user: this.mapDatabaseUserToProfile(newUser),
          message: 'User created successfully'
        };
      }
    } catch (error) {
      const err = extractError(error);
      this.logger.error(`User sync error:`, err.message);
      throw error;
    }
  }

  private mapDatabaseUserToProfile(dbUser: any) {
    return {
      uid: dbUser.id,
      email: dbUser.email,
