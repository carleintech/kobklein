...
async function testAuthService() {
  ...
  const mockWalletsService = {
    createWallet: jest.fn().mockResolvedValue({ id: 'mock-wallet-id', currency: 'HTG' }),
  } as any;
  ...
  const authService = new AuthService(jwtService, configService, mockWalletsService, mockEmailService);
  ...
  try {
    ...
    // Test role methods
    const hasRole = await authService.hasRole('test-user-id', 'INDIVIDUAL' as any);
