...
export class UsersService {
  ...
  async findAll(params?: {
    ...
    try {
      ...
      const sanitizedUsers = (data || []).map(user => this.sanitizeUserResponse(user));
  ...
  async updateProfile(userId: string, updateProfileDto: UpdateProfileDto): Promise<UserProfile> {
    ...
    try {
      ...
      if (error) {
        this.logger.error(`Error updating profile for ${userId}:`, error.message);
  ...
  async updateKYCStatus(
    ...
    try {
      ...
      if (error) {
        this.logger.error(`Error updating KYC status for ${userId}:`, error.message);
      ...
      this.logger.error(`KYC status update failed for ${userId}:`, error.message);
      throw error;
    }
  }

  async submitKycDocuments(userId: string, kycSubmissionDto: KycSubmissionDto): Promise<UserProfile> {
    ...
    try {
      ...
      if (error) {
        this.logger.error(`Error submitting KYC for ${userId}:`, error.message);
      ...
      this.logger.error(`KYC submission failed for ${userId}:`, error.message);
      throw error;
    }
  }

  async updateUserStatus(userId: string, isActive: boolean, reason?: string) {
    this.logger.log(`Updating user status: ${userId} to ${isActive ? 'active' : 'inactive'}`);
  ...
  async remove(id: string, reason?: string) {
    const result = await this.updateUserStatus(id, false, reason || 'Account deleted');
    return {
      success: true,
  ...
  async getUserStats(userId: string) {
    ...
    try {
      ...
      const totalBalance = user.wallets?.reduce((sum, wallet) => sum + wallet.balance, 0) || 0;
  ...
  private async getTransactionStats(userId: string, direction: 'sent' | 'received') {
    try {
      const column = direction === 'sent' ? 'sender_wallet_id' : 'receiver_wallet_id';
      ...
      if (error) {
        this.logger.warn(`Error getting ${direction} stats:`, error.message);
        return { totalAmount: 0, count: 0 };
      }

      const totalAmount = (data || []).reduce((sum, txn) => sum + parseFloat(txn.amount || '0'), 0);
      return { totalAmount, count: data?.length || 0 };
    } catch (error) {
      this.logger.warn(`Error calculating ${direction} stats:`, error.message);
      return { totalAmount: 0, count: 0 };
    }
  }
  ...
  private async getTotalTransactionCount(userId: string): Promise<number> {
    try {
      ...
      if (error) {
        this.logger.warn('Error getting transaction count:', error.message);
        return 0;
      }

      return count || 0;
    } catch (error) {
      this.logger.warn('Error calculating transaction count:', error.message);
      return 0;
    }
  }
  ...
  async submitKyc(userId: string, kycData: KycSubmissionDto) {
    ...
    try {
      ...
      if (docsError) {
        this.logger.error('Error inserting KYC documents:', docsError.message);
        throw new BadRequestException(`Failed to save KYC documents: ${docsError.message}`);
      }
      ...
      // Log the submission
      await this.logStatusChange(userId, UserStatus.PENDING, userId, 'KYC documents submitted');
      ...
      this.logger.error('Error in submitKyc:', error.message);
      throw error;
    }
  }
  ...
  async getAnalytics() {
    ...
    try {
      ...
      if (roleError) {
        this.logger.error('Error fetching role statistics:', roleError.message);
        throw new BadRequestException(`Failed to fetch role statistics: ${roleError.message}`);
      }
      ...
      if (kycError) {
        this.logger.error('Error fetching KYC statistics:', kycError.message);
        throw new BadRequestException(`Failed to fetch KYC statistics: ${kycError.message}`);
      }
      ...
      if (regError) {
        this.logger.error('Error fetching registration trends:', regError.message);
        throw new BadRequestException(`Failed to fetch registration trends: ${regError.message}`);
      }
      ...
      const kycCounts = kycStats?.reduce((acc, item) => {
        acc[item.kyc_status] = (acc[item.kyc_status] || 0) + 1;
        return acc;
      }, {}) || {};

      // Group registrations by day
      const registrationTrends = registrationData?.reduce((acc, item) => {
      ...
      return {
        ...
        data: {
          ...
          totalKycUsers: kycStats?.filter(k => k.kyc_status !== KycStatus.PENDING).length || 0,
        },
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
  ...
  private async logStatusChange(userId: string, status: UserStatus, adminId: string, reason?: string) {
    try {
      await this.supabase
        .from('audit_logs')
        .insert({
      ...
      this.logger.warn('Failed to log status change:', error.message);
    }
  }
}
